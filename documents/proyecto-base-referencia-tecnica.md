# 🧱 El proyecto base — referencia técnica

> Este documento describe **la API que ya está hecha** y que usás como punto de partida para el TP
> de Prompting. El objetivo del trabajo práctico y los ejercicios están en el
> [README principal](../README.md) y en la carpeta [`prompting/`](../prompting/00%20-%20README%20-%20Como%20usar%20este%20TP%20de%20Prompting.md).

Primero entendé cómo está armada la API, y después pedile a la IA que le agregue funcionalidad.

Es una API REST que hace CRUD de alumnos y cursos contra PostgreSQL, con una **arquitectura en
capas** (controller → service → repository) y una clase helper de acceso a datos (`DbPg`) que aísla
todo el boilerplate de PostgreSQL.
...
El código es intencionalmente verboso y autoexplicativo, con comentarios pensados para que entiendas
cada paso.

> 📖 En esta misma carpeta `documents/` hay un **documento de explicación** que detalla, paso a paso,
> cómo funciona esta arquitectura en capas y por qué está diseñada así. Ver [Documentación](#-documentación--guía-de-lectura).
...
![Servicio de PostgreSQL en Windows](images/services-postgress.jpg)
...
![Diagrama de arquitectura en capas](images/arquitectura.jpg)
...
|-----------|-------------|
| [Arquitectura en capas — Controller → Service → Repository → DbPg](server-capas-explicacion.md) | La arquitectura en capas, el flujo de un request, la lógica de negocio en el service (calcular edad, validar FK), la clase helper `DbPg` y sus 4 métodos, y las decisiones de diseño clave (Pool vs Client, queries parametrizadas `$1`, dotenv, entities). |

> 💡 Una vez que entendés la arquitectura, el siguiente paso es **agregarle funcionalidad vos**
> prompteando con IA de forma incremental → volvé a [`prompting/`](../prompting/00%20-%20README%20-%20Como%20usar%20este%20TP%20de%20Prompting.md).
